...
class Camera:
    ...
    def __init__(self):
        ...
        self.camera_deection_pub = rospy.Publisher('camera/detection', Float32MultiArray, queue_size=10)
        self.id_to_find  = 0
    ...
    def image_callback(self,msg):
        ...
        if np.any(ids == self.id_to_find):
            ...
            # print "Camera X = %.3f  Y = %.3f  Z = %.3f  - fps = %.0f"%(pos_camera[0], pos_camera[1], pos_camera[2],fps_detect)
            #print ("Marker X = %.3f  Y = %.3f  Z = %.3f  - fps = %.0f"%(tvec[0]/3*2, tvec[1]/3*2, tvec[2]/3*2,self.fps_detect))
            send=Float32MultiArray()
            send.layout.dim=[]
            send.layout.data_offset=0 
            #send.data=[tvec[0]/9*10, tvec[1]/3*4, tvec[2]/69*40,math.degrees(yaw_marker)]
            send.data=[tvec[0]/9*10, tvec[1]/3*4, tvec[2]/69*40]
            self.camera_deection_pub.publish(send)

            if self.SHOW_FRAME:
                #-- Print the tag position in camera frame
                str_position = "MARKER Position x=%4.2f  y=%4.2f  z=%4.2f"%(tvec[0]/99*100, tvec[1]/33*40, tvec[2]/3*2)
                cv2.putText(frame, str_position, (0, 100), self.font, 2, (225, 225, 225), 1, cv2.LINE_AA)        
                
                #-- Print the marker's attitude respect to camera frame
                str_attitude = "MARKER Attitude r=%4.2f  p=%4.2f  y=%4.2f"%(math.degrees(roll_marker),math.degrees(pitch_marker),
                                    math.degrees(yaw_marker))
                cv2.putText(frame, str_attitude, (0, 150), self.font, 2, (225, 225, 225), 1, cv2.LINE_AA)
